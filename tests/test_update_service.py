...
class UpdateServiceTest(unittest.TestCase):
    ...
    def test_docker_permission_preflight_fails_before_compose_changes(self) -> None:
        self._write_instance(tag="preview")
        docker = FakeDocker(access_error="docker_permission_denied")
        service = self._service(docker)

        with self.assertRaises(UpdateServiceError) as raised:
            service.run(
                UpdateRequest(data_dir=self.data_dir, channel="stable"),
                self.events.append,
            )

        self.assertEqual(raised.exception.code, "docker_permission_denied")
        self.assertEqual(docker.pulls, [])
        self.assertEqual(docker.ups, [])
        self.assertIn(
            "image: kromiose/nekro-agent:preview",
            (self.data_dir / "docker-compose.yml").read_text(encoding="utf-8"),
        )
