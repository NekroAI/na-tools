...
def _send_reply(sock: socket.socket, code: int) -> None:
    reply = (
        bytes([SOCKS_VERSION, code, 0x00, SOCKS_ATYP_IPV4])
...
def resolve_default_socks_bind_host() -> str:
    """Return the default SOCKS bind host for container host-gateway access.

    Docker Compose projects commonly run on user-defined bridge networks, where
    ``host.docker.internal`` may resolve to that network's gateway instead of
    the default ``docker0`` address. Binding all host interfaces keeps the
    control channel reachable while the SOCKS target whitelist and daemon HMAC
    still guard what the channel can do.
    """

    return DEFAULT_SOCKS_BIND_HOST
