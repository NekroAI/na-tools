...
_DOCKER_ACCESS_REASONS = {
    "docker_unavailable",
    "docker_not_running",
    "docker_permission_denied",
    "docker_socket_missing",
}
_UNAVAILABLE_REASON_MESSAGES = {
    "compose_missing": "未找到 Docker Compose 配置，请重新运行 na-tools bind。",
    "env_missing": "未找到实例环境配置，请重新运行 na-tools bind。",
}


def unavailable_reason_message(code: str) -> str:
    """Return a safe user-facing message for an unavailable daemon reason."""

    if code in _DOCKER_ACCESS_REASONS:
        return docker_access_error_message(cast(DockerAccessErrorCode, code))
    return _UNAVAILABLE_REASON_MESSAGES.get(code, "NA-Tools daemon 当前不可用。")
    ...
    def current_instance(self) -> dict[str, object]:
        ...
        return {
            ...
            "docker": {
                ...
                "access_error": unavailable_reason
                if unavailable_reason in _DOCKER_ACCESS_REASONS
                else None,
            },
        }
